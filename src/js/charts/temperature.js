var Temperature = function (wrapper, i, branches, domain) {
  var debug = false;
  ...
  var yScale = d3.scaleLinear().domain([0, 40]);
  ...
  var fullText =
    "Temperature is an important parameter in the real-time water quality assessment. Because temperature differs between the three branches of the river we are monitoring, temperature is treated differently in each branch’s water quality model. For example, the Main Stem location is colder than the other branches because it is influenced by Lake Michigan.";
  var textYOffset = 35;
  var textXOffset = 50;
  ...
  function registerObservers() {
    d3.select("#branch").on("change.temperature", updateBranch);
  }
  ...
  function filterValue(obj) {
    return obj.value !== null && obj.date >= this.startDate;
  }
  ...
  function updateEventRects0() {
    var startDate = activeBranch.values[0].date;
    var endDate = activeBranch.values[activeBranch.values.length - 1].date;
    var width =
      (xScale(endDate) - xScale(startDate)) / activeBranch.values.length;
    var rects = gMain.selectAll("rect.event").data(activeBranch.values);
    rects
      .enter()
      .append("rect")
      .attr("class", "event")
      .attr("fill", "transparent")
      .attr("x", function (d, i) {
        return xScale(activeBranch.values[0].date) + i * width;
      })
      .attr("y", 0)
      .attr("width", width)
      .attr("height", mainBox.outer.height)
    ...
    gMain.on("mouseleave", hideTooltip);
  ...
  function showTooltip(d) {
    ...
    tooltip.select('[data-tooltip-role="title"]').text("Temperature");
    tooltip.select('[data-tooltip-role="value"]').html(`${d.tooltipValue}°C`);
    tooltip
